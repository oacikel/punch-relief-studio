...
1. **Import** -- pick one of the three built-in samples (no upload needed),
   ...
   once).
2. **Orient** -- rotate/pan/zoom to the viewpoint you want. Only what's
   visible from this one viewpoint becomes the pattern; back and hidden
   surfaces are not captured (see the in-app note on this step).
3. **Create relief** -- adjust height levels (3-8), relief intensity,
...
4. **Height levels** -- see how much of the pattern each level covers and
   which needle setting it maps to, plus a warning if any region is too
   small to punch reliably.
5. **Yarn colors** -- choose single-color, color-by-height, or (when the
   model has material/texture data) source-material color extraction, and
   edit swatches/names.
6. **Preview** -- compare pattern views (combined/color-only/height-only/
...
7. **Export** -- set the physical project size, export SVG/PNG/print-PDF,
   save project settings as JSON, and manage your calibration profile
   (including a printable calibration strip).
...
print the calibration strip (Export step), punch each labeled block on
scrap fabric with the corresponding needle setting, measure the pile
height with a ruler, and enter the measurements back into the profile.
Save the profile so it's remembered next time (stored locally in your
browser; export it as JSON to back it up or move it to another device).
